...
export const useRecipes = create((set, get) => ({
  ...
  fetchRecipes: async (page = 1) => {
    ...
    try {
      const response = await getRecieps(
        `https://api.punkapi.com/v2/beers?page=${page}&per_page=${get().limit}`,
      );
  ...
  deleteAll: () =>
    set((state) => {
      ...
      const forRender = getAllRecipes.filter(
        (item) => !selectedRecipes.includes(item),
      );
      const deleteItems = getAllRecipes.filter((item) =>
        selectedRecipes.includes(item),
      );
      return {
        recipes: forRender,
        selectedRecipes: [],
        deleteItemsAll: [...get().deleteItemsAll, ...deleteItems],
      };
    }),
  setPage: (num) => {
